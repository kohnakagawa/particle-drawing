...
MouseHandle::MouseHandle(float center_x, float center_y, float center_z,
                         float eDist, float fov) {
  eyeDistance  = eDist;
  ...
  center2eye[1] = eyeDistance * std::sin(phi) * std::sin(theta);
  center2eye[2] = eyeDistance * std::cos(theta);

  ebase_z[0] = std::cos(phi) * std::sin(theta - 0.5 * M_PI);
  ebase_z[1] = std::sin(phi) * std::sin(theta - 0.5 * M_PI);
  ebase_z[2] = std::cos(theta - 0.5 * M_PI);
