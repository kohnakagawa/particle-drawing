...
class MouseHandle {
  struct quaternion {
    ...
    explicit quaternion(const float a[]) : t(0.0), x(a[0]), y(a[1]), z(a[2]) {}
    quaternion(const float t_, const float x_, const float y_, const float z_):t(t_), x(x_), y(y_), z(z_) {}
    quaternion(const float radian, float axis[]) {
    ...
    const quaternion operator * (const quaternion& obj) const {
